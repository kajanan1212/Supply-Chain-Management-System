...
router.get('/reports', (req, res) => {
    const sql = [
        ...
        "select truck_number, first_name as store_name, ifnull(truck_used_hours, 0) as used_hr from((select * from(select truck_id, sum(time_to_sec(timediff(end_time, start_time)) / (60 * 60)) as truck_used_hours from truck left outer join truck_schedule using(truck_id) group by truck_id) as truck_used_hr left outer join truck using(truck_id)) as truck left outer join owns using(truck_id)) left outer join store using(store_id) order by truck_used_hours desc limit 10"
    ]
    // const sql = ["SET @UsrYear = '2022'", "SET @UsrQuarter = 1", "select  makedate(@UsrYear, 1) + interval @UsrQuarter quarter - interval 1 quarter", "select  makedate(@UsrYear, 1) + interval @UsrQuarter quarter - interval 1 day"]
    db.query(sql.join(';'), (err, result) => {
        // console.log(sql)
